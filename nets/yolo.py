...
from .backbonev5 import v5
from .v5mamba import v5mamba
import math
...
class YOLOXHead(nn.Module):
    def __init__(self, num_classes, width=1.0, in_channels=[256, 512, 1024], act="silu", depthwise=True):
        super().__init__()
        Conv = DWConv if depthwise else BaseConv
...
class YOLOPAFPN(nn.Module):
    def __init__(self, depth = 2.0, width = 2.0, in_features = ("dark3", "dark4", "dark5"), in_channels = [256, 512, 1024], depthwise = False, act = "silu"):
        super().__init__()
        Conv                = DWConv if depthwise else BaseConv
        self.backbone       = v5mamba(depth, width,depthwise = depthwise, act = act)
